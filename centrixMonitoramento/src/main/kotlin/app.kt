...
fun main() {
    ...
    while (true) {
        ...
        if (autenticado) {
            ...
            val componentesExistentes = repositorioComponentes.obterComponentesExistentes()

            println("Componentes Disponíveis para Monitoramento:")
            for ((index, componente) in componentesExistentes.withIndex()) {
                println("${index + 1}. ${componente.nome} - ${componente.descricao}")
            }

            val componentesMonitorados = mutableListOf<Componente>()
            ...
            while (true) {
                ...
                if (escolha in 1..componentesExistentes.size) {
                    val componenteEscolhido = componentesExistentes[escolha - 1]
