...
function listarLateral() {
    var nivelAcesso = sessionStorage.NivelAcesso;
    var barraLateral = document.getElementById("lateral_barra");
    var listaLateral = [];
    if (nivelAcesso >= 2) {
        listaLateral.push("Visoes")
        listaLateral.push("Main");
        listaLateral.push("Gráfico");
    ...
    listaLateral.push("Help Desk");
    listaLateral.push("Sair");

    for (let i = 0; i < listaLateral.length; i++) {
        if (listaLateral[i] == "Main") {
            barraLateral.innerHTML += `
                <a href="dashboard_main.html">
                    <div id="option1" class="barraLateral_option cursor">
                        <img src="../assets/imgs/casinha.png" alt="home">
                    </div>
                </a>
            `;
            ...
            `;
        } else if (listaLateral[i] == "Funcionário") {
            barraLateral.innerHTML += `
                <a href="dashboard_funcionarios.html">
                    <div id="option4" class="barraLateral_option cursor">
                        <img src="../assets/imgs/funcionariosCinza.png" alt="Funcionários"/>
                    </div>
                </a>
            `;
            ...
            `;
        } else if (listaLateral[i] == "Visoes"){
            barraLateral.innerHTML += `
                <div id="option9" class="barraLateral_option cursor primeiro">
                    <img  onclick = "virarSeta()" class="setaBaixo" src="../assets/imgs/Seta.png" alt="Expandir lateral">
                </div>
        `;
        }
    }
...
function irMonitoramento(id) {
    if (id == 0) alert("Selecione um computador para monitorar"); else window.location = "dashboard_maquina.html";
}
function virarSeta(){
    let setaDireita = document.getElementById("option9");

    if (setaDireita.style.transform == "rotate(-90deg)" ){
        setaDireita.style.transform = "rotate(0deg)";
        desaparecerOpcoes();
    }else{
        setaDireita.style.transform = "rotate(-90deg)";
        aparecerOpcoes();
    }
function aparecerOpcoes(){
    let setaDireitaContainer = document.getElementById("containerVisoes")
    setaDireitaContainer.style.visibility = "visible"
    setaDireitaContainer.style.left = "60px"
}
function desaparecerOpcoes(){
    let setaDireitaContainer = document.getElementById("containerVisoes")
    setaDireitaContainer.style.left = "-1000px"
    setaDireitaContainer.style.visibility = "hidden"
}
